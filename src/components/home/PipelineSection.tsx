...
function vy(i: number) { return 160 + i * 51; }

function lpath(i: number) {
  const y = vy(i);
  return `M ${SX} ${y} C ${SX+120} ${y} ${HX-130} ${HY} ${HX-HR} ${HY}`;
}
function rpath(i: number) {
  const y = vy(i);
  return `M ${HX+HR} ${HY} C ${HX+130} ${HY} ${OX-120} ${y} ${OX} ${y}`;
}
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: 144, height: 144 }}>
      <motion.div animate={{ rotate: 360 }} transition={{ duration: 24, repeat: Infinity, ease: "linear" }}
        className="absolute inset-0 rounded-full" style={{ border: "1px solid rgba(0,212,255,0.2)" }} />
      <motion.div animate={{ rotate: -360 }} transition={{ duration: 15, repeat: Infinity, ease: "linear" }}
        className="absolute rounded-full" style={{ inset: 16, border: "1px dashed rgba(124,58,237,0.35)" }} />
      <motion.div animate={{ rotate: 360 }} transition={{ duration: 8, repeat: Infinity, ease: "linear" }}
        className="absolute rounded-full" style={{ inset: 30, border: "1px solid rgba(0,255,156,0.25)" }} />
...
export default function PipelineSection() {
  ...
  const inView = useInView(sectionRef, { margin: "-15% 0px -15% 0px", once: false });

  // Start auto-play when section enters view; reset when it leaves
  useEffect(() => {
    if (inView) {
      setStep(1);
      setActive(true);
  ...
  // Advance step every 2.2 s while active; pause at step 5
  useEffect(() => {
    if (!active) return;
    const t = setInterval(() => {
      setStep(s => {
        if (s >= 5) { setActive(false); return 5; }
        return s + 1;
      });
    }, 2200);
  ...
  return (
    <section ref={sectionRef} style={{ background: "#0B1525", minHeight: "100vh", position: "relative", display: "flex", flexDirection: "column" }}>
      ...
      {/* Header */}
      <div className="relative z-10 px-8 sm:px-12 pt-14 pb-4 flex items-start justify-between">
        <div>
        ...
        <div className="flex items-center gap-1.5 mt-1.5">
          {STEPS.map((_, i) => (
            <button key={i} onClick={() => { setStep(i + 1); setActive(i + 1 < 5); }}
              className="rounded-full transition-all duration-300 cursor-pointer"
              style={{ width: step === i + 1 ? 20 : 7, height: 7,
      ...
      <div className="hidden lg:block relative flex-1 px-6 sm:px-10 pb-10 pt-2" style={{ minHeight: 480 }}>
        <div className="relative flex items-center w-full h-full" style={{ minHeight: 480 }}>
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none"
            viewBox="0 0 1000 524" preserveAspectRatio="none" style={{ overflow: "visible" }}>
            <defs>
              {SOURCES.map((s, i) => (
            ...
            {SOURCES.map((s, i) => {
              ...
              return (
                <g key={`lp${i}`}>
                  ...
                  <path d={d} fill="none" stroke={`url(#gl${i})`} strokeWidth="1.8" strokeLinecap="round"
            ...
            {SOURCES.map((s, i) => {
              ...
              return (
                <motion.div key={i}
                  ...
                  style={{ background: on ? `rgba(${toRgb(s.color)},0.09)` : "rgba(255,255,255,0.025)",
                    border: `1px solid ${on ? `rgba(${toRgb(s.color)},0.3)` : "rgba(255,255,255,0.06)"}`,
                  ...
                  {on && <div className="shrink-0 w-1.5 h-1.5 rounded-full" style={{ background: s.color, boxShadow: `0 0 5px ${s.color}` }} />}
                </motion.div>
              );
            })}
          </div>
            ...
            <AnimatePresence>
              {step === 5 && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}
                  transition={{ duration: 0.35 }} className="flex gap-2.5">
                  {[{ icon: Cloud, label: "Cloud", color: "#3B82F6" }, { icon: Server, label: "On-Premises", color: "#00D4FF" }].map((d, i) => (
            ...
            {OUTPUTS.map((o, i) => {
              ...
              return (
                <motion.div key={i}
                  ...
                  transition={{ duration: 0.45, delay: on ? i * 0.06 : 0 }}
                  className="flex items-center gap-2.5 rounded-xl px-3 py-2.5"
                  style={{ background: on ? `rgba(${toRgb(o.color)},0.09)` : "rgba(255,255,255,0.025)",
                    border: `1px solid ${on ? `rgba(${toRgb(o.color)},0.3)` : "rgba(255,255,255,0.06)"}`,
                    transition: "background 0.4s, border-color 0.4s" }}>
                  {on && <div className="shrink-0 w-1.5 h-1.5 rounded-full" style={{ background: o.color, boxShadow: `0 0 5px ${o.color}` }} />}
                  <div className="flex-1 min-w-0">
      ...
      <div className="relative z-10 px-8 sm:px-12 pb-12">
        <AnimatePresence mode="wait">
          <motion.div key={step}
            ...
            <h3 className="font-serif font-bold mb-1.5"
